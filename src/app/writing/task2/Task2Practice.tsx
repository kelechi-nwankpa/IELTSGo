...
export function Task2Practice({ promptId, title, prompt, topic, testType }: Task2PracticeProps) {
  ...
  const taskTypeLabel = testType === 'academic' ? 'Task 2 Academic' : 'Task 2 General';
  const otherTypeUrl =
    testType === 'academic' ? '/writing/task2?type=general' : '/writing/task2?type=academic';
  const otherTypeLabel =
    testType === 'academic' ? 'Switch to General Training' : 'Switch to Academic';
      ...
      <div className="mx-auto max-w-4xl px-4">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Write an essay of at least 250 words</p>
          </div>
        ...
        <div className="space-y-6">
          <WritingPrompt title={title} prompt={prompt} topic={topic} taskType={taskTypeLabel} />
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          {Object.entries(evaluation.criteria).map(([key, criterion]) => (
            <div key={key} className="rounded-lg border border-gray-200 bg-white p-5">
              ...
              <SafeText as="p" className="mb-3 text-sm text-gray-600">
                {criterion.summary}
              </SafeText>
