...
import { parseAndValidate, validateWritingEvaluation } from './output-validator';
...
${sanitizedResponse}`;
  ...
  const validationResult = parseAndValidate(textContent.text, validateWritingEvaluation);
  ...
  return { evaluation: validationResult.data as WritingEvaluation, tokensUsed };
}
